...
`POST /open/api/v1/resource/knowledge/note/batch-add` 支持 `fid` 参数，用于将笔记加入知识库的指定文件夹：

```json
{
  "topic_id": "gJV8EpYp",
  "note_ids": ["1234567890123456789"],
  "fid": "4726291"
}
```
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `fid` | string | 知识库文件夹 ID，从 Web URL `?fid=XXX` 获取，**必须传字符串** |

⚠️ **互斥规则**：`parent_id`（子笔记）和知识库不能共存——有父子关系的笔记调用 `batch-add` 会静默失败（`success_count=0`）。知识库内的文件夹路径需通过 `fid` 参数实现，不能用 `parent_id`。

⚠️ **获取 fid**：在 Get笔记 Web 端进入目标文件夹，URL 中 `?fid=XXX` 即为文件夹 ID。
