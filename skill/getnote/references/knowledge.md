...
> 🛑 **公开 API 不支持知识库子文件夹定位**：笔记通过 `batch-add` 或 `save` 的 `topic_id` 加入知识库后，固定出现在知识库**根目录**。`fid`、`folder_id` 等参数均会被 API **静默忽略**（不报错、不生效）。如需将笔记归入子文件夹（如"行业趋势 > IPC日报"），只能在 Get笔记 App 中手动拖拽。
