...
# 凭证优先级: 环境变量 > E:/AI智能体/CODE/api_credentials.json > 硬编码默认值
def _load_client_id():
    """从统一凭证文件或环境变量加载 client_id"""
    env_val = os.environ.get("GETNOTE_CLIENT_ID", "")
    if env_val:
        return env_val
    cred_file = Path("E:/AI智能体/CODE/api_credentials.json")
    if cred_file.exists():
        try:
            cfg = json.loads(cred_file.read_text(encoding="utf-8"))
            return cfg.get("getnote", {}).get("client_id", "")
        except Exception:
            pass
    return ""

DEFAULT_CLIENT_ID = _load_client_id() or "cli_a1b2c3d4e5f6789012345678abcdef90"
