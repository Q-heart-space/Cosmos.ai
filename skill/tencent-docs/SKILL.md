...
depends: slide-mcp, references/auth.md, references/workflows.md, smartcanvas/entry.md, references/slideengine_references.md
triggers:
  - 腾讯文档
  - 创建文档
  - 新建文档
...
## 盲区与自审
- 本技能的已知盲区：能力完全依赖腾讯文档 MCP 服务与 Token，未授权（400006）、VIP/积分不足（400007/400008）时受限；不同品类工具分散在多个服务（tencent-docs 主服务 vs slide-mcp），PPT 精细编辑必须走 slide-mcp 的 slide_* 工具，用 doc_*/通用工具改 PPT 会返回错误内容；存在"不支持的能力需静默上报"机制，说明工具集有覆盖缺口；`create_smartcanvas_by_mdx` 不支持 parent_id。
- 自审方式：遇到 400006/400007/400008 错误码时，先区分是鉴权/权限/积分问题而非操作错误；对 PPT 编辑类需求，检查是否误用了 doc_*/通用工具而非 slide-mcp；功能在工具列表找不到时，走 report_unsupported_feature 而非强行模拟。
