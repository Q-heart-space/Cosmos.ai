...
## 2026-09-17（第三次提交）— ai-drift-guard v1.5：分层修正为三层 + S9 升为硬拦截

- **分层由两层改为三层**：A 硬拦截 / B 确定性注入 / C 纯提示。旧版把「能不能强制」和「能不能保证模型看见」混为一谈
- **给出判据**：触发谓词必须是该接缝可观测量的全函数。这是 A 的准入门槛，并解释了为什么某些信号**永远**到不了 A
- **S9 由提示层升为 Tier A**：旧版写「提示词做不到」只对了一半——写在提示词里做不到，缝2 的 `reject` 做得到
- 新增 `references/first-principles.md`（第一原理推导）、`references/prior-art.md`（先例调查，坦白列出同类项目）、`references/adoption.md`（采用指南，开门即说「你可能不需要本技能」）
- `selftest.mjs` 由 13 项扩到 24 项，新增 S9 的**误报回归**断言
